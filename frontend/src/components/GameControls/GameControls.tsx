...
export function GameControls({ disabled = false, onChoose }: GameControlsProps) {
  ...
  return (
    <div className="mt-8 flex gap-2.5" aria-label="Price prediction controls">
      <button type="button" disabled={disabled} onClick={() => onChoose("up")} className={`${buttonClass} bg-up text-[#06251b] enabled:hover:bg-[#63e5b5]`}>
        <span aria-hidden="true">↑</span> UP
      </button>
      <button type="button" disabled={disabled} onClick={() => onChoose("down")} className={`${buttonClass} bg-down text-[#2d080b] enabled:hover:bg-[#ff8994]`}>
