...
test("loads authoritative resolved status using only the bet ID", async () => {
  ...
  const fetchMock = vi
    .fn()
    .mockResolvedValue({ ok: true, status: 200, json: async () => resolved });
...
test.each([403, 404])(
  ...
  async (status) => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://example.test/bets/bet%20with%20spaces",
      expect.any(Object),
    );
  },
);
...
test("loads the authenticated user's completed bets", async () => {
  ...
  const bet = {
    ...
    startPrice: "100",
    endPrice: "101",
    startEventTimestamp: "2026-08-20T12:00:00Z",
    resolutionTargetTimestamp: "2026-08-20T12:01:00Z",
    endEventTimestamp: "2026-08-20T12:01:01Z",
    createdAt: "2026-08-20T12:00:01Z",
  };
  const fetchMock = vi
    .fn()
    .mockResolvedValue({ ok: true, json: async () => ({ bets: [bet] }) });
  vi.stubGlobal("fetch", fetchMock);
  await expect(getCompletedBets()).resolves.toEqual([bet]);
  expect(fetchMock).toHaveBeenCalledWith(
    "https://example.test/bets",
    expect.objectContaining({
...
test("reconstruction availability expires ten hours after the bet's source start", () => {
  ...
  expect(canReconstructBet(bet, Date.parse("2026-08-20T11:59:54.999Z"))).toBe(
    true,
  );
  expect(canReconstructBet(bet, Date.parse("2026-08-20T11:59:55Z"))).toBe(
    false,
  );
});
...
test("history separates reconstructable bets from the older count", () => {
  const current = {
    betId: "current",
    startEventTimestamp: "2026-08-20T11:00:00Z",
  } as ResolvedBet;
  const old = {
    betId: "old",
    startEventTimestamp: "2026-08-20T01:00:00Z",
  } as ResolvedBet;
  expect(
    reconstructableBetHistory(
      [current, old],
